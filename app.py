...
from vm2026_logic import (
    ...
    load_json_bytes, download_json, fill_try_luck
)
...
def init_session():
    for k, v in {"participant_data": load_local(LOCAL_PARTICIPANT_FILE, new_prediction("")), "actual_data": load_local(LOCAL_ACTUAL_FILE, new_actual_results()), "participant_ui_version": 0, "actual_ui_version": 0, "play_luck_song": False}.items():
...
def import_box(label: str, target: str, key: str):
    ...
    if uploaded and st.button("Importer JSON", key=f"{key}_button"):
        try: data = load_json_bytes(uploaded)
        except Exception as exc:
            st.error(f"Kunne ikke lese JSON: {exc}"); return
...
def try_luck_button(label: str, target: str, key: str):
    if st.button(label, key=key, type="primary", help="Fyller ut alle kamper basert på vektede odds/styrker. Gir nytt forslag hver gang."):
        if target == "participant":
            ...
            st.session_state.participant_data["participant"] = name
            st.session_state.participant_ui_version += 1; clear_widget_keys("p_")
        else:
...
def score_inputs(prefix: str, team_a: str, team_b: str, current: dict, allow_draw_winner: bool = False) -> dict:
    ...
    if allow_draw_winner and team_a and team_b and ga == gb:
        options = ["", team_a, team_b]; old = current.get("winner", "")
    ...
    for group in GROUPS:
        with st.expander(f"Gruppe {group}", expanded=group in ["A", "B"]):
            for m in [x for x in GROUP_MATCHES if x["group"] == group]:
                mk = str(m["match_no"]); st.write(f"Kamp {mk}")
                target[mk] = score_inputs(f"{prefix}_{key_name}_{mk}", m["team_a"], m["team_b"], normalize_score(target.get(mk)), False)
...
def render_tables_and_slots(data: dict, prefix: str):
    ...
    for i, group in enumerate(GROUPS):
        with cols[i % 3]:
            st.markdown(f"#### Gruppe {group}"); st.dataframe(q["tables"][group].drop(columns=["Seed"]), hide_index=True, use_container_width=True)
    st.markdown("### Beste tredjeplasser")
    thirds = q["thirds"].copy(); thirds.insert(0, "Rang", range(1, len(thirds)+1)); thirds["Videre"] = ["Ja" if i < 8 else "Nei" for i in range(len(thirds))]
    ...
    for slot, allowed in slot_allowed_map().items():
        options = [""] + [g for g in allowed if g in adv]; old = overrides.get(slot, "")
        c1,c2,c3 = st.columns([2,3,3]); c1.write(f"**{slot}**"); c2.write(f"Auto: {auto.get(slot,'')}")
...
def render_knockout_inputs(data: dict, key_name: str, prefix: str):
    ...
    for phase in PHASE_ORDER:
        ...
        for no, m in sorted([(no,x) for no,x in bracket.items() if x["phase"] == phase]):
            ...
            target[str(no)] = score_inputs(f"{prefix}_{key_name}_{no}", m["team_a"], m["team_b"], normalize_score(target.get(str(no))), True)
        bracket = compute_bracket(data.get("group_scores", {}), data.get("third_slot_overrides", {}), target)
...
def admin_mode():
    ...
    with tab4:
        if st.button("Lagre fasit lokalt", key=f"{prefix}_save_actual"): save_local(LOCAL_ACTUAL_FILE, actual); st.success(f"Lagret til {LOCAL_ACTUAL_FILE}")
